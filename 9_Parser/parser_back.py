...
parse1_consts = [('TOP=S', 0, 5), ('NP', 0, 2), ('NNP', 0, 1), ('NNP', 1, 2), ('VP', 2, 4), ('VBZ', 2, 3),
                 ('NP=NNP', 3, 4), ('.', 4, 5)]
parse1_words = ['Ms.', 'Haag', 'plays', 'Elianti', '.']


# parse = "(TOP(S(NP(DT A)(NN record)(NN date))(VP(VBZ has)(RB n't)(VP(VBN been)(VP(VBN set))))(. .)))"
...
class Parser:
    ...
    @staticmethod
    def remove_chain_rules_ori(constituents):
        """
    ...
    def remove_chain_rules(constituents):
        ...
        Sort the given constituent list to have an order of depth first traversal.
        Remove chain rules from the given constituent list.
        input e.g. [('TOP', 0, 5), ('S', 0, 5),  .. ]
        output e.g. [('TOP=S', 0, 5), ..]
...
p = Parser()
tree = "(TOP(S(NP(NP(RB Not)(PDT all)(DT those))(SBAR(WHNP(WP who))(S(VP(VBD wrote)))))(VP(VBP oppose)(NP(DT the)(NNS changes)))(. .)))"
